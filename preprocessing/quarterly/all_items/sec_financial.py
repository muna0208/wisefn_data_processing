...
logger = logging.getLogger('quarterly_secfin_all')
...
def generate_quarterly_sec_fin_all(conf, to_csv=True, to_clickhouse=True):
    ...
    # check master
    for sec in secs[:]:
        logger.info(f'processing quarterly_secfin_all: {sec}')
        try:
            ...
            if to_clickhouse:
                write_clickhouse(con_, 'quarterly_secfin_all', 'QUARTERLY', data)
        
        except Exception as e:
            logger.error(f'{e}')


def generate_quarterly_sec_fin_all_ch(conf, to_clickhouse=True):
    con_ = ClickHouseConnection(
        host=conf['ClickHouse']['host'],
        user=conf['ClickHouse']['user'],
        password=conf['ClickHouse']['password'],
...
if __name__ == '__main__':
    ...
    # generate_quarterly_sec_fin_all(conf)
    generate_quarterly_sec_fin_all_ch(conf)
