...
logger = logging.getLogger('quarterly_cmp_fin_all')
...
def generate_quarterly_cmp_fin_all(conf, to_csv=True, to_clickhouse=True):
    ...
    for cmp in cmps[:]:
        ...
        try:
            ...
            data = data.rename_axis('YYMM').reset_index().drop_duplicates(subset=['YYMM'], keep='first').set_index('YYMM')
            data[data.columns[22:]] = data[data.columns[22:]].astype('float')
            
            if to_csv:
                write_csv(path, cmp, data)
            if to_clickhouse:
                write_clickhouse(con_, 'quarterly_cmpfin_all', 'QUARTERLY', data)

        except Exception as e:
            logger.error(f'{e}')

def generate_quarterly_cmp_fin_all_ch(conf, start_dt='00000000', end_dt='99999999', to_clickhouse=True):
    con_ = ClickHouseConnection(
        host=conf['ClickHouse']['host'],
        user=conf['ClickHouse']['user'],
        password=conf['ClickHouse']['password'],
    ...
    for cmp in cmps[:]:
        logger.info(f'processing quarterly_cmpfin_all: {cmp}')
        try:
            ...
            data = pd.concat([sep,con])
            if len(data) == 0:
                logger.warning(f'empty {termaccts[0]}, {cmp}')
                continue
...
if __name__ == '__main__':
    ...
    # generate_quarterly_cmp_fin_all(conf)
    generate_quarterly_cmp_fin_all_ch(conf)
