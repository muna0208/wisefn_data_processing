...
def convert_comp_fin_data_by_company(comp_code, daily_data, comp_fin_data, comp_info):
    ...
    comp_rows = comp_fin_data["YYMM"].drop_duplicates().sort_values()
    ...
    for k in TermAcctMethod:
        ...
        logger.info(filepath + TermAcctDir[k.name].value + "/" + 'py/' + "A" + comp_code + ".csv")
        mat = empty_matrix.copy()
        if os.path.exists(filepath + TermAcctDir[k.name].value + "/" + 'py/'+ "A" + comp_code + ".csv"):
            mat = pd.read_csv(filepath + TermAcctDir[k.name].value + "/" + 'py/'+ "A" + comp_code + ".csv", 
                    index_col=0, 
                    dtype='str')
        ...
        if len(indices) > 0:
            mat.loc[indices, ["MKT_TYP", "GICS_CD", "WI26_CD"]] = one_comp_info.loc[indices, ["MKT_TYP", "GICS_CD", "WI26_CD"]]
            mat.loc[:, ['MKT_TYP','GICS_CD','WI26_CD']] = mat.loc[:, ['MKT_TYP','GICS_CD','WI26_CD']].bfill()
            logger.info(f'indices for MKT_TYP {k.name} {indices}')
            logger.info(f'{mat.loc[indices, :]}, {one_comp_info.loc[indices,["MKT_TYP", "GICS_CD", "WI26_CD"]]}')
    ...
    for acct in TermAcctMethod:
        ...
        mat_by_acct[acct].to_csv(filepath + TermAcctDir[acct.name].value + '/py' + "/" + "A" + comp_code + ".csv")
...
def convert_comp_fin_data_by_item(daily_data, comp_fin_data):
    ...
    for i in range(len(ITEMNAME_NEED_2)):
        ...
        for acct in TermAcctMethod:
            ...
            if os.path.exists(by_ITEM_path + "/" + TermAcctDir[acct.name].value + "/" + 'py/' + item_name + ".csv"):
                mat = pd.read_csv(by_ITEM_path + "/" + TermAcctDir[acct.name].value + "/" +  'py/' +item_name + ".csv",
                        dtype='str', 
                        index_col=0)
            else:
                mat = empty_matrix
            ...
            if not os.path.exists(by_ITEM_path + "/"+ TermAcctDir[acct.name].value):
                os.makedirs(by_ITEM_path + "/" +  'py/' + TermAcctDir[acct.name].value)
            mat_by_acct[acct].to_csv(by_ITEM_path + "/"+ TermAcctDir[acct.name].value + "/" +  'py/' + item_name+ ".csv1")
...
def Data_conversion(data_path=INPUTPATH, date='20210226'):
    data_path = INPUTPATH
    date = '20210226'
    ...
    dates = allfolder[-50:]
    for d in allfolder:
        daily_data = get_daily_data(data_path, d)
        dates = [d]
