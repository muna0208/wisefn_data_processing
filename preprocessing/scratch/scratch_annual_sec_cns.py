...
for sec in secs:
    for dt in dates:
        if not os.path.exists(path + '/' + f'{dt}'  + '/' + '99ALL_CUM' + "/" + f'{sec}' + ".csv"):
            continue
        org_ = pd.read_csv(path + '/' + f'{dt}'  + '/' + '99ALL_CUM' + "/" + f'{sec}' + ".csv", 
                index_col=0, 
                dtype='str')
                
        new_ = pd.read_csv(path + '/' + f'{dt}'  + '/' + '99ALL_CUM' + "/" + f'{sec}' + ".csv1", 
                index_col=0, 
                dtype='str')
        ...
        click = con_.get_client().query_dataframe(f"SELECT * FROM {table_name} WHERE SEC_CD = '{sec}' AND TERM_TYP='ALL_CUM' AND CNS_DT='{dt}'")
        click = click.drop_duplicates(subset=['SEC_CD', 'YEAR'],keep='last')

        click.YEAR = click.YEAR.apply(int)
