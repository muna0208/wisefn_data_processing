...
for sec in secs:
    if not os.path.exists(path + '/' + '99YEARLY' + "/" + f'{sec}' + ".csv"):
        continue
    org_ = pd.read_csv(path + '/' + '99YEARLY' + "/" + f'{sec}' + ".csv", 
            index_col=0, 
            dtype='str')
            
    new_ = pd.read_csv(path +  '/'  + '99YEARLY' + '/' + f'{sec}' + ".csv1", 
            index_col=0, 
            dtype='str')
    ...
    click = con_.get_client().query_dataframe(f"SELECT * FROM {table_name} WHERE SEC_CD = '{sec}' AND TERM_TYP='YEARLY'")
