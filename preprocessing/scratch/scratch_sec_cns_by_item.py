...
for itm in  np.array(CNSITEMNAME_NEED_2)[np.random.choice(len(CNSITEMNAME_NEED_2), size=5, replace=False)]:
    for dt in np.array(dates)[np.random.choice(len(dates), size=50, replace=False)]:
        if not os.path.exists(path + '/' + f'{dt}' + '/' + TermAcctDir[TermAcctMethod.SEP_CUM.name].value + "/" + f'{itm}' + ".csv"):
            continue
        org_ = pd.read_csv(path + '/' + f'{dt}' + '/' + TermAcctDir[TermAcctMethod.SEP_CUM.name].value + "/" + f'{itm}' + ".csv", 
                index_col=0, 
                dtype='str')                
        if not os.path.exists(path + '/' + f'{dt}' + '/' + TermAcctDir[TermAcctMethod.SEP_CUM.name].value + "/" + f'{itm}' + ".csv1"):
            continue
        new_ = pd.read_csv(path +  '/' + f'{dt}' + '/' +  TermAcctDir[TermAcctMethod.SEP_CUM.name].value + "/" + f'{itm}' + ".csv1", 
                index_col=0, 
                dtype='str')
        ...
        click = con_.get_client().query_dataframe(f"SELECT * FROM {table_name} WHERE ITEM_NM = '{itm}' AND TERM_TYP='{TermAcctMethod.SEP_CUM.value}' AND CNS_DT='{dt}'")
