...
for sec in np.array(secs)[np.random.choice(len(secs), size=10, replace=False)]:
    for dt in np.array(dates)[np.random.choice(len(dates), size=50, replace=False)]:
        if not os.path.exists(path + '/' + f'{dt}' + '/' + TermAcctDir[TermAcctMethod.SEP_CUM.name].value + "/" + f'{sec}' + ".csv"):
            continue
        org_ = pd.read_csv(path + '/' + f'{dt}' + '/' + TermAcctDir[TermAcctMethod.SEP_CUM.name].value + "/" + f'{sec}' + ".csv", 
                index_col=0, 
                dtype='str')
                
        if not os.path.exists(path + '/' + f'{dt}' + '/' + TermAcctDir[TermAcctMethod.SEP_CUM.name].value + "/" + f'{sec}' + ".csv1"):
            continue
        new_ = pd.read_csv(path +  '/' + f'{dt}' + '/' +  TermAcctDir[TermAcctMethod.SEP_CUM.name].value + "/" + f'{sec}' + ".csv1", 
                index_col=0, 
                dtype='str')
        ...
        click = con_.get_client().query_dataframe(f"SELECT * FROM {table_name} WHERE SEC_CD = '{sec}' AND TERM_TYP='{TermAcctMethod.SEP_CUM.value}' AND CNS_DT='{dt}'")
        ...
        # new.Cash_Dividend_Comm[new.Cash_Dividend_Comm > '0']
