...
logger = logging.getLogger('by_sector_sec_fin_all')
...
def generate_sec_fin_by_sec_all(conf, to_csv=True, to_clickhouse=True):
    ...
    path = conf['Path']['by_sector_sec_fin_all_path']
    ...
    for sec_cd in sector.index[:].sort_values():
        try:
            ...
            cols = findata.columns.intersection(list(zip(items.ITEM_CD, items.ITEM_TYP)))
            findata = findata.loc[pd.IndexSlice[:,:], cols].rename(columns=colname_map).reindex(list(items.ITEM_NM_ENG), axis='columns')
            ...
            for term_acct in TermAcctMethod:
                ...
                if to_clickhouse:
                    write_clickhouse(con_, 'secfin_bysec_all', term_acct, sec_cd, findata_, sector)
        except Exception as e:
            logger.error(f'{e}')
...
if __name__ == '__main__':
    ...
    generate_sec_fin_by_sec_all(conf)
