...
logger = logging.getLogger('by_company_cmp_fin_all')
...
def generate_cmp_fin_by_cmp_all_mp(conf, to_csv=True, to_clickhouse=True, cmps=[]):
    ...
    items = get_items(db)
    item_unit = get_item_unit(items)
    company = get_company(db)

    for cmp_cd in cmps:
        try:
            findata, fininfo, finprd = get_cmp_data_tuple_to_merge(db, cmp_cd, items)
            if not isinstance(findata, pd.DataFrame) and findata == None:
                continue
            ...
            for term_acct in TermAcctMethod:
                ...
                if to_clickhouse:
                    write_clickhouse(con_, 'cmpfin_bycmp_all', term_acct, cmp_cd, findata_.copy(), fininfo_.copy(), finprd, company)
...
def get_items(db):
    ...
    items.loc[:, 'ITEM_NM_ENG'] = items.loc[:,'ITEM_NM_ENG'].apply(
                                                                lambda x: x.strip()
                                                                    ...
                                                                    .replace('+', '_and_')
                                                                    .replace(':', '.'))
    items = items.loc[items.ITEM_TYP=='M', :]
    return items


def get_item_unit(items, unit='천원'):
...
def get_cmp_data_tuple_to_merge(db, cmp_cd, items):
    ...
    findata_cols = findata.columns.intersection(list(zip(items.ITEM_CD, items.ITEM_TYP)))
    findata = findata.loc[pd.IndexSlice[:,:], findata_cols].rename(columns=colname_map).reindex(list(items.ITEM_NM_ENG), axis='columns')
    ...
    if len(float_cols) > 0:
        # for other nullable int => fininfo.loc[:, float_cols] = fininfo.loc[:, float_cols].astype(pd.Int64Dtype())
    ...
    return findata, fininfo, finprd


def generate_cmp_fin_by_cmp_all(conf, start_dt=None, end_dt=None, to_csv=True, to_clickhouse=True, num_procs=8):
    con_ = ClickHouseConnection(
        host=conf['ClickHouse']['host'],
        user=conf['ClickHouse']['user'],
        password=conf['ClickHouse']['password'],
    ...
    for k in cmps_map.keys():
        proc_map[k] = mp.Process(target=generate_cmp_fin_by_cmp_all_mp, args=(conf, to_csv, to_clickhouse, cmps_map[k]))
        proc_map[k].start()
...
if __name__ == '__main__':
    ...
    generate_cmp_fin_by_cmp_all(conf, to_csv = False)
