...
logger = logging.getLogger('annual_secfin_all')
...
def generate_annual_sec_fin_all(conf, to_csv=True, to_clickhouse=True):
    ...
    for sec in secs[:]:
        try:
            ...
            if to_clickhouse:
                write_clickhouse(con_, 'annual_secfin_all', 'YEARLY', data)

        except Exception as e:
            logger.error(f'{e}')

def generate_annual_sec_fin_all_ch(conf, to_clickhouse=True):
    con_ = ClickHouseConnection(
        host=conf['ClickHouse']['host'],
        user=conf['ClickHouse']['user'],
        password=conf['ClickHouse']['password'],
    ...
    for sec in secs[:]:
        try:
            ...
            data = data.rename_axis('YEAR').reset_index().drop_duplicates(subset='YEAR', keep='last').set_index('YEAR')
...
if __name__ == '__main__':
    ...
    # generate_annual_sec_fin_all(conf)
    generate_annual_sec_fin_all_ch(conf)
