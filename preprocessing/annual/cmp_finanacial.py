...
logger = logging.getLogger('annual_cmpfin')
...
def generate_annual_cmp_fin(conf, to_csv=True, to_clickhouse=True):
    ...
    for cmp in cmps:
        ...
        try:
            ...
            # TODO looks like we do not need this code any more
            ...
            if to_clickhouse:
                write_clickhouse(con_, 'annual_cmpfin', 'YEARLY',  data)

        except Exception as e:
            logger.error(f'{e}')


def generate_annual_cmp_fin_ch(conf, start_dt='00000000', end_dt='99999999', to_clickhouse=True):
    con_ = ClickHouseConnection(
        host=conf['ClickHouse']['host'],
        user=conf['ClickHouse']['user'],
        password=conf['ClickHouse']['password'],
    ...
    for cmp in cmps:
        ...
        try:
            ...
            data.pop('YYMM')
            data[data.columns[24:]] = data[data.columns[24:]].astype('float')
...
if __name__ == '__main__':
    ...
    #generate_annual_cmp_fin(conf)
    generate_annual_cmp_fin_ch(conf)
