...
logger = logging.getLogger('annual_seccns')
...
def generate_annual_sec_cns(conf, start_dt='00000000', end_dt='99999999', to_csv=True, to_clickhouse=True):
    ...
    for sec in secs[:]:
        logger.info(f'processing annual_seccns: {sec}')
        for dt in date_list[:]:
            try:
                data = pd.DataFrame()
                # calc settlement month Dec('12') and others differently
                # use SEP_CUM
                target_path = path + '/' + dt + '/' +  termaccts[0]
                if not os.path.exists(target_path + '/' + sec + '.csv1'):
                    continue
                logger.debug(target_path + '/' + sec + '.csv')
                ...
                if to_clickhouse:
                    write_clickhouse(con_, 'annual_seccns', 'ALL_CUM', sec, dt, data)

            except Exception as e:
                logger.error(f'{e}')

def generate_annual_sec_cns_ch(conf, to_clickhouse=True):
    con_ = ClickHouseConnection(
        host=conf['ClickHouse']['host'],
        user=conf['ClickHouse']['user'],
        password=conf['ClickHouse']['password'],
    ...
    for sec in secs[:]:
        ...
        try:
            ...
            data = sep
            data = data[(data.TERM_TYP == termaccts[0])]
            data = data.rename_axis('YEAR').reset_index().sort_values(['YEAR', '_ts']).drop_duplicates(subset=['YEAR'], keep='last').set_index('YEAR')
            data.index = [idx[:-2] if idx[-2:] == '12' else str(int(idx[:-2]) - 1) for idx in data.index]

            if len(data) == 0:
                logger.warning(f'empty {sec}')
                continue

            data[data.columns[5:]] = data[data.columns[5:]].astype('float').copy()
            if to_clickhouse:
                write_clickhouse(con_, 'annual_seccns', 'ALL_CUM', sec, data)
...
if __name__ == '__main__':
    ...
    start_dt = '20210401'
    end_dt = '20210601'
    generate_annual_sec_cns(conf, start_dt, end_dt, False, True)
    # generate_annual_sec_cns_ch(conf, start_dt, end_dt)
