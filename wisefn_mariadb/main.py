...
logger = logging.getLogger('wisefn_mariadb main')
...
def main(config_file, start_dt, end_dt, last_date_only=True, table_names=[]):
    ...
    logger.debug(f'processing dates: {dates}')
    ...
    for table_name in table_names[:]:
        db.bulk_write_financial_csv_to_mariadb(
            ...
            conf['Path']['txt_path'],
            dates, 
            chunksize=eval(conf['MariaDB']['chunksize']))
...
if __name__ == '__main__':
    ...
    main(config_file, start_dt, end_dt, last_date_only)
